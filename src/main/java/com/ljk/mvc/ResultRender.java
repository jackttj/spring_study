...
import com.ljk.util.ValidateUtil;
...
//        boolean isJson = method.isAnnotationPresent(ResponseBody.class);
        ResponseBody deMAnno = method.getAnnotation(ResponseBody.class);
        boolean isJson = deMAnno.value() == "js";
        if (isJson) {
            log.info("is a json?");
            // 设置响应头
            ...
            if (result instanceof ModelAndView) {
                log.info("this is a good pro,and you hava a big bra");
                ...
                log.info("is a string?");
                path = (String) result;
            } else {
                throw new RuntimeException("返回类型不合法");
            }
            try {
//                req.getRequestDispatcher("/static/" + path).forward(req, resp);
